...
Day 8 Answer Key - reconciles diagnostic_logs rows against what
actually exists in S3. Two independent sources of truth (the
database's claim, and S3's reality) can drift apart; this script
...
def extract_s3_key(file_url: str) -> str:
    ...
    Strips the scheme and bucket name, keeping only the actual object
    key - the part that has to match what list_objects_v2 returns.
    """
...
def list_s3_keys(bucket_name: str, prefix: str) -> set[str]:
    ...
    automatically follows the continuation token for anything beyond
    that, so this stays correct as the bucket grows (see the Research
    Prompts in day08_demo_and_challenge.md).
    """
    s3_client = boto3.client("s3")
    paginator = s3_client.get_paginator("list_objects_v2")

    #Accumulate all the keys in a set so we can do set operations later
    keys: set[str] = set()
...
#async function to fetch all diagnostic logs from the database
...
#main method to compare the s3 keys against the database rows and print the results
async def main() -> None:
    ...
    print("\n== Broken (database row, no matching file) ==")
    if not broken:
        print("  None found.")
    for log in broken:
        print(f"  DiagnosticLog {log.id}: {log.file_url}")
    ...
    print("\n== Orphaned (file in S3, no matching database row) ==")
